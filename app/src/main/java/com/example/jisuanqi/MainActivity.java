...
public class MainActivity extends Activity {
    ...
    private OnClickListener actionPerformed = new OnClickListener() {
        public void onClick(View v) {
            ...
            if("0123456789.lnlog+-×÷√^".indexOf(command) != -1 && tip_lock) {
                Tipcommand[tip_i] = command;
                tip_i++;
            }
            //若输入正确，则将输入信息显示到显示器上
            if("0123456789.lnlog+-×÷√^".indexOf(command) != -1
                    && tip_lock) { //共25个按键
                print(command);
     ...
     * 只包含0123456789.lnlog+-×÷√^的是合法的str，返回true
     * 包含了除0123456789.lnlog+-×÷√^以外的字符的str为非法的，返回false
     */
    private boolean right(String str) {
        ...
        for(i = 0;i < str.length();i++) {
            if(str.charAt(i)!='0' && str.charAt(i)!='1' && str.charAt(i)!='2' &&
                    ...
                    str.charAt(i)!='n' && str.charAt(i)!='o' && str.charAt(i)!='a' &&
    ...
    private void TipChecker(String tipcommand1,String tipcommand2) {
        ...
        //定义存储字符串中最后一位的类型
            ...
            if(tipcommand1.compareTo(".") == 0) {
                ...
                tiptype1 = 6;
            } else if("logln".indexOf(tipcommand1) != -1) {
                tiptype1 = 7;
            }
            //定义欲输入的按键类型
            if(tipcommand2.compareTo("(") == 0) {
                ...
                tiptype2 = 6;
            } else if("logln".indexOf(tipcommand2) != -1) {
                tiptype2 = 7;
            }
        ...
        if(Tipcode1 == 0 && tipcommand2.compareTo(".") == 0) {
            ...
            for(int i = 0;i < tip_i;i++) {
                ...
                if(Tipcommand[i].compareTo("log") == 0 ||
     ...
     * 3、错误提示          showError(int code ,String str)  将错误返回
     */
    public class calc {
        public calc(){
        ...
        public void process(String str) {
            ...
            String num;//记录数字，str以+-×÷gl√^分段，+-×÷()gl√^字符之间的字符串即为数字
            weight = new int[MAXLEN];
            number = new double[MAXLEN];
            operator = new char[MAXLEN];
            String expression = str;
            StringTokenizer expToken = new StringTokenizer(expression,"+-×÷gl√^");
            int i = 0;
